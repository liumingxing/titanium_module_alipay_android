...
#define TAG "com.mamashai.alipay"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> AlipayAndroid_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("AlipayAndroid.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::AlipayAndroidBindings::lookupGeneratedInit(
...
static void AlipayAndroid_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(AlipayAndroid_getBinding)->GetFunction());
}

static void AlipayAndroid_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::AlipayAndroidBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry AlipayAndroidBinding = {
	"com.mamashai.alipay",
	AlipayAndroid_init,
	AlipayAndroid_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_com_mamashai_alipay_AlipayAndroidBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.mamashai.alipay", &AlipayAndroidBinding);
	titanium::KrollBindings::addExternalLookup(&(::AlipayAndroidBindings::lookupGeneratedInit));
}
